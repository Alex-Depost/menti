...
async def test_get_business_metrics():
    with patch("src.routers.metrics_router.get_mentors", new_callable=AsyncMock) as mock_get_mentors, \
         patch("src.routers.metrics_router.get_users", new_callable=AsyncMock) as mock_get_users:
...
async def test_metrics_endpoint_integration(test_client):
    with patch("src.repository.mentor_repository.get_mentors", new_callable=AsyncMock) as mock_get_mentors, \
         patch("src.repository.user_repository.get_users", new_callable=AsyncMock) as mock_get_users:
        
        # Mock repository responses
        mock_get_mentors.return_value = ([], 15)  # 15 total mentors
        mock_get_users.return_value = ([], 25)    # 25 total students
        ...
        # Check that metrics are in the response
        assert "mentors_count 15" in response.text
        assert "students_count 25" in response.text